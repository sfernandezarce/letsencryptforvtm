...
ZCLI=$(locate zcli 2>/dev/null | egrep "/zxtm-11.1/bin/zcli$")
...
case "$CERTTYPE" in
  ecc)
    ...
    CERTDIR=$ACMEHOME/mycerts/${CERTFILE}_${CERTTYPE}
    ;;
  rsa)
    ACMEKEY="--keylength 2048"
    CERTDIR=$ACMEHOME/mycerts/${CERTFILE}
    ;;
  *)
...
#$ACMEHOME/acme.sh $TEST $ACMEOPTIONS $ACMEACTION -d ${CERTFILE} $ACMEKEY -d www.${CERTFILE} $ACMEKEY -d mobile.${CERTFILE} $ACMEKEY -d *.${CERTFILE} $ACMEKEY
$ACMEHOME/acme.sh $TEST $ACMEOPTIONS $ACMEACTION -d ${CERTFILE} $ACMEKEY -d *.${CERTFILE} $ACMEKEY
...
echo "Catalog.SSL.Certificates.setRawCertificate x.${CERTFILE} \"$crt\" " > $CERTDIR/zcli_${CERTFILE}.script
$ZCLI $CERTDIR/zcli_${CERTFILE}.script
if [ $? -ne 0 ]; then
  echo "Catalog.SSL.Certificates.importCertificate x.${CERTFILE} { private_key: \"$key\", public_cert: \"$crt\" }" > $CERTDIR/zcli_${CERTFILE}.script
  $ZCLI $CERTDIR/zcli_${CERTFILE}.script
fi
